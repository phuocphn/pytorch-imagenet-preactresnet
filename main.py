...
model_names = sorted(name for name in models.__dict__
    ...
    and callable(models.__dict__[name])) + ["preactresnet_18"]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    if args.arch == "preactresnet_18":
        model = PreActResNet18(dataset="ImageNet")
    else:
        if args.pretrained:
    ...
    if (args.distributed and dist.get_rank() == 0) or not args.distributed:
        ...
        for idx in range(device_ids):
            print("[%s]" % (idx), torch.cuda.get_device_name(idx), " Allocated: ", round(torch.cuda.memory_allocated(idx)/1024**3,1), 'GB')
    ...
    if args.lr_scheduler == "cos":
        ...
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=eval(lr_scheduler), gamma=0.1)
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        transforms.Compose([
            transforms.Resize(256), #add resize here ...
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    with torch.no_grad():
        ...
        if (args.distributed and dist.get_rank() == 0) or not args.distributed:
            ...
            writer.add_scalar('Accuracy@1/test', top1.avg, epoch)
            writer.add_scalar('Accuracy@5/test', top5.avg, epoch)
